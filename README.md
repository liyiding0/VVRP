# Python 路由器軟體，界面為Cisco/華為風格的 CLI 解釋器

這是一個小型 Cisco/華為CLI風格的路由器軟件（含命令行解釋器），命令行有如下特點：
...
`show host interfaces` is retained as a hidden/debug command family for inspecting host OS interfaces.
